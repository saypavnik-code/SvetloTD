// MenuScene.ts — Stage 10 Phase 5: Full cinematic redesign.
// Dark fantasy citadel · animated hex grid · ember particles · gothic rune frame
...
export class MenuScene extends Phaser.Scene {
  ...
  private _buildBg(): void {
    ...
    for (let r = 340; r > 0; r -= 22) {
      g.fillStyle(COLORS.amberDeep, 0.022 * (1 - r / 340));
      g.fillEllipse(GAME_WIDTH / 2, GAME_HEIGHT * 0.28, r * 2.4, r);
    }
  }
  ...
  private _drawHex(dt: number): void {
    ...
    for (const c of this._hexCells) {
      ...
      g.lineStyle(1, COLORS.amberDeep, 0.03 + glow * 0.055);
      const pts = [];
      for (let i = 0; i < 6; i++) { const a = Math.PI / 6 + (Math.PI / 3) * i; pts.push({ x: c.cx + R * Math.cos(a), y: c.cy + R * Math.sin(a) }); }
      g.strokePoints(pts as Phaser.Types.Math.Vector2Like[], true);
      if (glow > 0.94) { g.fillStyle(COLORS.amberWarm, 0.028); g.fillPoints(pts as Phaser.Types.Math.Vector2Like[], true); }
    }
  }
  ...
  private _buildHorizon(): void {
    ...
    for (let i = 0; i < 28; i++) { const t = i / 28; g.fillStyle(0x0A0806, t * t * 0.60); g.fillRect(0, hy - 90 + i * (90 / 28), GAME_WIDTH, 90 / 28 + 1); }
    g.lineStyle(1, COLORS.amberDeep, 0.40); g.beginPath(); g.moveTo(0, hy); g.lineTo(GAME_WIDTH, hy); g.strokePath();
    for (let i = 0; i < 18; i++) {
  ...
  private _buildTowers(): void {
    ...
    for (const t of this._towers) {
      ...
      if (w > 14) g.fillRect(t.x - 1, hy - t.h * 0.38, 2, Math.round(t.h * 0.08));
    }
  }
  ...
  private _buildFrame(): void {
    ...
    g.lineStyle(1, COLORS.amberDeep,   0.18); g.strokeRoundedRect(lx+10, ty+10, pw-20, ph-20, 8);
    for (const [dcx, dcy] of [[lx,ty],[lx+pw,ty],[lx,ty+ph],[lx+pw,ty+ph]] as [number,number][]) {
      ...
      g.lineStyle(1, COLORS.amberGlow, 0.65); g.strokePoints([{x:dcx,y:dcy-s},{x:dcx+s,y:dcy},{x:dcx,y:dcy+s},{x:dcx-s,y:dcy}] as Phaser.Types.Math.Vector2Like[], true);
    }
    const divY = ty + 175;
  ...
  private _buildPlayBtn(): void {
    ...
    const draw = (hover: boolean): void => {
      ...
      if (hover) {
        ...
        g.lineStyle(1, COLORS.amberGlow, 0.50); g.strokeRoundedRect(3,3,bw-6,bh-6,8);
      } else {
  ...
  private _buildFooter(): void {
    const cx = GAME_WIDTH/2, cy = GAME_HEIGHT/2;
    try {
      const s = localStorage.getItem('best_result');
      if (s) {
        ...
        const line = b.isVictory
          ...
          : `Лучший рубеж  ·  Волна ${b.wavesCompleted}  ·  ❤ ${b.livesRemaining}  ·  ☠ ${b.enemiesKilled}`;

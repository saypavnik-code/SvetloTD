...
export class HUD {
  ...
  private _build(): void {
    ...
    bg.fillRect(0, 0, gw, BAR_H);
    // Subtle top sheen
    bg.fillStyle(0x221C14, 0.55);
    bg.fillRect(0, 0, gw, 7);
    // Inner bevel top
    bg.lineStyle(1, 0x342B1E, 0.6);
    bg.beginPath(); bg.moveTo(0, 1); bg.lineTo(gw, 1); bg.strokePath();

    // Primary amber accent bottom line
    bg.lineStyle(2, COLORS.amberWarm, 0.88);
    bg.beginPath(); bg.moveTo(0, BAR_H - 1); bg.lineTo(gw, BAR_H - 1); bg.strokePath();
    // Glow halo under accent
    bg.lineStyle(3, COLORS.amberDeep, 0.22);
    bg.beginPath(); bg.moveTo(0, BAR_H + 1); bg.lineTo(gw, BAR_H + 1); bg.strokePath();

    // Field separator (right of game field)
    bg.lineStyle(2, COLORS.walnutDark, 0.9);
    bg.beginPath(); bg.moveTo(FW, 0); bg.lineTo(FW, BAR_H); bg.strokePath();
    bg.lineStyle(1, COLORS.amberDeep, 0.7);
    bg.beginPath(); bg.moveTo(FW + 2, 0); bg.lineTo(FW + 2, BAR_H); bg.strokePath();

    // ── Wave countdown sub-bar ────────────────────────────────────────────
    this._waveBarFill = s.add.graphics().setDepth(DEPTH.HUD - 1);

    // ── Section: Lives ───────────────────────────────────────────────────
    const S1 = 14;
    this._pill(bg, S1 - 4, 11, 90, 28, COLORS.danger, 0.14, 0.32);
    this._livesIcon = s.add.text(S1 + 1, BAR_H / 2, '♥', {
      fontFamily: FONT, fontSize: '16px', color: COLORS.textDanger_css, fontStyle: 'bold',
    }).setOrigin(0, 0.5).setDepth(DEPTH.HUD);
    this._livesNum = s.add.text(S1 + 24, BAR_H / 2, `${STARTING_LIVES}`, {
      fontFamily: FONT, fontSize: '19px', fontStyle: 'bold', color: COLORS.textDanger_css,
      shadow: { offsetX: 0, offsetY: 0, color: '#B34838', blur: 9, fill: true },
    }).setOrigin(0, 0.5).setDepth(DEPTH.HUD);
    this._livesNum.setData('ox', S1 + 24);
    s.add.text(S1 + 62, BAR_H / 2 + 2, 'ЖИЗ', {
      fontFamily: FONT, fontSize: '8px', letterSpacing: 1, color: COLORS.textMuted_css,
    }).setOrigin(0, 0.5).setDepth(DEPTH.HUD);
    ...
    s.add.text(S2 + 1, BAR_H / 2, '◆', {
      fontFamily: FONT, fontSize: '14px', color: COLORS.amberWarm_css,
    }).setOrigin(0, 0.5).setDepth(DEPTH.HUD);
    this._goldDisplay = this._economy.gold;
    this._goldNum = s.add.text(S2 + 22, BAR_H / 2, `${this._economy.gold}`, {
      fontFamily: FONT, fontSize: '19px', fontStyle: 'bold', color: COLORS.amberGlow_css,
      shadow: { offsetX: 0, offsetY: 0, color: COLORS.amberWarm_css, blur: 11, fill: true },
    }).setOrigin(0, 0.5).setDepth(DEPTH.HUD);
    s.add.text(S2 + 68, BAR_H / 2 + 2, 'ЗОЛОТО', {
      fontFamily: FONT, fontSize: '8px', letterSpacing: 1, color: COLORS.textMuted_css,
    }).setOrigin(0, 0.5).setDepth(DEPTH.HUD);
    ...
    s.add.text(S3 + 1, BAR_H / 2 - 1, '🪵', {
      fontFamily: FONT, fontSize: '13px',
    }).setOrigin(0, 0.5).setDepth(DEPTH.HUD);
    this._lumberNum = s.add.text(S3 + 26, BAR_H / 2, '0', {
      fontFamily: FONT, fontSize: '19px', fontStyle: 'bold', color: '#7CBA5C',
      shadow: { offsetX: 0, offsetY: 0, color: '#2A6A10', blur: 8, fill: true },
    }).setOrigin(0, 0.5).setDepth(DEPTH.HUD);
    s.add.text(S3 + 54, BAR_H / 2 + 2, 'ЛЕС', {
      fontFamily: FONT, fontSize: '8px', letterSpacing: 1, color: COLORS.textMuted_css,
    }).setOrigin(0, 0.5).setDepth(DEPTH.HUD);

    // ── Centre: Wave label (field centred) ───────────────────────────────
    const CX = FW / 2;
    this._waveText = s.add.text(CX, BAR_H / 2 - 5, `Волна 1 / ${WAVES.length}`, {
      fontFamily: FONT, fontSize: '17px', fontStyle: 'bold', color: COLORS.textPrimary_css,
      shadow: { offsetX: 0, offsetY: 0, color: '#000', blur: 7, fill: true },
    }).setOrigin(0.5).setDepth(DEPTH.HUD);
    this._phaseTag = s.add.text(CX, BAR_H / 2 + 13, 'ПОДГОТОВКА', {
      fontFamily: FONT, fontSize: '8px', letterSpacing: 4, color: COLORS.textMuted_css,
    }).setOrigin(0.5).setDepth(DEPTH.HUD);

    // ── Right: Speed pills ───────────────────────────────────────────────
    const PW_B = 42, PH_B = 28, GAP = 6;
    const totalW = 3 * PW_B + 2 * GAP;
    const px0 = GAME_WIDTH - totalW - 18;
    const py0 = (BAR_H - PH_B) / 2;

    s.add.text(px0 - 10, BAR_H / 2, 'СКОР', {
      fontFamily: FONT, fontSize: '8px', letterSpacing: 1, color: COLORS.textMuted_css,
    ...
    [1, 2, 3].forEach((spd, i) => {
      ...
      const lbl = s.add.text(bx + PW_B / 2, py0 + PH_B / 2, `×${spd}`, {
        fontFamily: FONT, fontSize: '13px', fontStyle: 'bold', color: COLORS.textSecondary_css,
      ...
      const zone = s.add.zone(bx, py0, PW_B, PH_B).setOrigin(0)
        .setInteractive({ useHandCursor: true }).setDepth(DEPTH.HUD + 2);
      zone.on('pointerover', () => {
        if (GameSpeed.speed !== spd) {
          bgG.clear();
          bgG.fillStyle(COLORS.amberDeep, 0.30); bgG.fillRoundedRect(bx, py0, PW_B, PH_B, 6);
          bgG.lineStyle(1, COLORS.amberWarm, 0.50); bgG.strokeRoundedRect(bx, py0, PW_B, PH_B, 6);
        }
      });
      zone.on('pointerout', () => this._redrawSpeed(i, GameSpeed.speed === spd));
      zone.on('pointerdown', () => {
        GameSpeed.set(spd);
        this._speedBtns.forEach((b, j) => this._redrawSpeed(j, b.speed === spd));
      });
    });
  }
  ...
  private _pill(
    ...
    g.fillStyle(color, fillAlpha); g.fillRoundedRect(x, y, w, h, 6);
    g.lineStyle(1, color, strokeAlpha); g.strokeRoundedRect(x, y, w, h, 6);
  }
  ...
  setLives(n: number): void {
    ...
    this._scene.tweens.add({
      targets: this._livesNum, x: { from: ox - 5, to: ox },

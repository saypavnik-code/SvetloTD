// ─────────────────────────────────────────────────────────────────────────────
// PauseOverlay.ts — Pause panel with volume sliders (Phase 4 extraction).
// ─────────────────────────────────────────────────────────────────────────────
...
export class PauseOverlay {
  ...
  private _build(): void {
    ...
    const makeSlider = (
      ...
      this._root.add(s.add.text(sliderX, sy, label, {
      ...
      const zone = s.add.zone(sliderX, sy + 10, BAR_W, BAR_H + 24)
    ...
    this._root.add(r1);
    this._root.add(s.add.text(cx, btnY + 19, 'Продолжить', {
      fontFamily: FONT, fontSize: '14px',
      color: COLORS.walnutDark_css, fontStyle: 'bold',
    }).setOrigin(0.5));
    const z1 = s.add.zone(cx - 100, btnY, 200, 38)
      .setOrigin(0).setInteractive({ useHandCursor: true });
    z1.on('pointerdown', () => { this._sfx?.uiClick(); this.toggle(); });
    this._root.add(z1);
    ...
    this._root.add(r2);
    this._root.add(s.add.text(cx, btnY2 + 17, 'В главное меню', {
      fontFamily: FONT, fontSize: '13px', color: COLORS.walnut_css,
    }).setOrigin(0.5));
    const z2 = s.add.zone(cx - 100, btnY2, 200, 34)
      .setOrigin(0).setInteractive({ useHandCursor: true });
    z2.on('pointerdown', () => {
      this._sfx?.uiClick();
      this._scene.cameras.main.fadeOut(300, 240, 238, 233);
      this._scene.cameras.main.once('camerafadeoutcomplete',
        () => this._scene.scene.start('MenuScene'));
    });
    this._root.add(z2);
  }
}
// FloatingTextPool.ts — Scale-pop, size variants, stroke shadow.
...
export class FloatingTextPool {
  ...
  show(x: number, y: number, message: string, color?: string, size: 'sm'|'md'|'lg' = 'md'): void {
    ...
    t.setStyle({ fontSize, strokeThickness: size==='lg' ? 4 : 3 });
    t.setText(message).setColor(color ?? COLORS.amberWarm_css);
    t.setPosition(x - t.width/2, y - 16).setAlpha(1).setScale(0.6).setVisible(true).setActive(true);
    this._scene.tweens.add({
      targets: t, scaleX:{from:0.6,to:1.0}, scaleY:{from:0.6,to:1.0},
      duration: 120, ease: 'Back.easeOut',
      onComplete: () => {
        this._scene.tweens.add({
          targets: t, y: t.y+FLOAT_DY, alpha: 0, duration: DURATION, ease: 'Cubic.easeOut',

...
(function (TargetPriority) {
    TargetPriority[TargetPriority["FIRST"] = 0] = "FIRST";
    TargetPriority[TargetPriority["LAST"] = 1] = "LAST";
    TargetPriority[TargetPriority["STRONGEST"] = 2] = "STRONGEST";
    TargetPriority[TargetPriority["WEAKEST"] = 3] = "WEAKEST";
    TargetPriority[TargetPriority["NEAREST"] = 4] = "NEAREST";
})(TargetPriority || (TargetPriority = {}));
const TARGET_TICK = 200;
const BODY_SIZE = 34;
export class Tower {
    constructor(scene, towerId, col, row, projPool, activeEnemies, invested) {
        // World position
        ...
        Object.defineProperty(this, "gridCol", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "gridRow", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "towerId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "totalInvested", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "targetPriority", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: TargetPriority.FIRST
        });
        // Stats tracking
        ...
        Object.defineProperty(this, "_spawnTimer", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0.25
        ...
        Object.defineProperty(this, "_rangeCircle", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "_bladesAngle", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        ...
        Object.defineProperty(this, "_targetTick", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "_fireCooldown", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        ...
        Object.defineProperty(this, "_activeEnemiesFn", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    _redrawRange(selected) {
        const g = this._rangeCircle;
        g.clear();
        const a = selected ? 0.30 : 0.14;
        g.lineStyle(1, this.data.color, a);
        g.strokeCircle(0, 0, this.data.range);
        g.fillStyle(this.data.color, a * 0.10);
        g.fillCircle(0, 0, this.data.range);
    }
    showRange(visible, selected = false) {
        this._selected = selected;
        this._rangeCircle.setVisible(visible);
        if (visible)
            this._redrawRange(selected);
    }
    // Tab — show/hide ghost range ring at low alpha
    showRangeGhost(visible) {
        if (this._selected)
            return; // don't override selection ring
        this._rangeCircle.setVisible(visible);
        if (visible) {
            const g = this._rangeCircle;
            g.clear();
    ...
    update(delta) {
        ...
        if (this.towerId === 'slow_t1')
            this._bladesAngle += gd * 0.003;
        if (this._spawnTimer > 0)
            this._spawnTimer = Math.max(0, this._spawnTimer - gd / 1000);
        // Tick active buffs (dt in seconds)
        this._tickBuffs(gd / 1000);
        if (this._targetTick >= TARGET_TICK) {
            this._targetTick = 0;
            this._findTarget();
        }
    ...
    get isBuffed() { return this._buffs.length > 0; }
    _tickBuffs(dtSecs) {
    ...
    _findTarget() {
        ...
        const candidates = this._activeEnemiesFn().filter(e => {
            if (!e.isActive || e.isDead)
                return false;
            if (e.def.isFlying && !this.data.canTargetAir)
                return false;
            if (!e.def.isFlying && !this.data.canTargetGround)
                return false;
        ...
        if (this._target?.isActive && !this._target.isDead && candidates.includes(this._target))
            return;
        this._target = this._pickTarget(candidates);
    }
    _pickTarget(list) {
        const d2 = (e) => { const dx = e.x - BASE_CENTER_X, dy = e.y - BASE_CENTER_Y; return dx * dx + dy * dy; };
    ...
    _fire() {
        ...
        proj.init(this.x, this.y, this._target, this.data.damage, this.data.damageType, this.data.splash, this.data.color, this._activeEnemiesFn(), 320);
    ...
    drawTo(g) {
        ...
        // Amber base plate
        g.fillStyle(COLORS.amberDeep, 0.20);
        g.fillRect(cx - h - 2, cy - h - 2, (BODY_SIZE + 4) * bounce, (BODY_SIZE + 4) * bounce);
        ...
        switch (this.towerId) {
            ...
            case 'arrow_t2':
                ...
                g.fillRect(cx - h + 4, cy - h + 4, h * 2 - 8, h * 2 - 8);
                [[-h, -h], [h, -h], [-h, h], [h, h]].forEach(([dx, dy]) => { g.fillStyle(c, 1); g.fillRect(cx + dx - 3, cy + dy - 3, 6, 6); });
                g.fillStyle(COLORS.walnutDark, 0.8);
                g.fillRect(cx - 2, cy - h - 4, 4, 8);
                break;
            case 'cannon_t1':
            case 'cannon_t2':
                g.fillStyle(c, 0.85);
                g.fillRect(cx - h, cy - h, h * 2, h * 2);
                g.fillStyle(COLORS.walnutDark, 0.4);
                g.fillCircle(cx, cy, h - 4);
                g.lineStyle(2, COLORS.walnutDark, 0.7);
                g.strokeRect(cx - h, cy - h, h * 2, h * 2);
                g.fillStyle(COLORS.walnutDark, 1);
                g.fillRect(cx - 3, cy - h - 6, 6, 10);
                break;
            case 'magic_t1':
            case 'magic_t2': {
                const pts = [];
                for (let i = 0; i < 6; i++) {
                    ...
                    pts.push({ x: cx + Math.cos(a) * h, y: cy + Math.sin(a) * h });
                }
                g.fillStyle(c, 0.9);
                g.fillPoints(pts, true);
                g.lineStyle(1.5, COLORS.amberBright, 0.6);
                g.strokePoints(pts, true);
                g.fillStyle(COLORS.amberGlow, 0.9);
            ...
            case 'slow_t1':
                ...
                g.lineStyle(1.5, COLORS.seaDark, 0.8);
                g.strokeCircle(cx, cy, h);
                for (let i = 0; i < 4; i++) {
                    const a = this._bladesAngle + Phaser.Math.DegToRad(i * 90);
                    const bx = cx + Math.cos(a) * 12, by = cy + Math.sin(a) * 12;
                    const pa = a + Math.PI / 2;
                    const pts = [{ x: bx + Math.cos(pa) * 2, y: by + Math.sin(pa) * 2 }, { x: bx + Math.cos(a) * 6, y: by + Math.sin(a) * 6 }, { x: bx - Math.cos(pa) * 2, y: by - Math.sin(pa) * 2 }, { x: bx - Math.cos(a) * 3, y: by - Math.sin(a) * 3 }];
                    g.fillStyle(COLORS.seaLight, 0.9);
                    g.fillPoints(pts, true);
                }
                g.fillStyle(COLORS.seaDark, 1);
                g.fillCircle(cx, cy, 4);
                break;
            case 'acid_t1': {
                const pts = [{ x: cx, y: cy - h }, { x: cx + h, y: cy }, { x: cx, y: cy + h }, { x: cx - h, y: cy }];
                g.fillStyle(c, 0.85);
                g.fillPoints(pts, true);
                g.lineStyle(1.5, COLORS.success, 0.6);
                g.strokePoints(pts, true);
                g.fillStyle(COLORS.successSoft, 0.8);
                g.fillCircle(cx, cy - h + 5, 4);
                break;
            }
            default:
                g.fillStyle(c, 0.88);
    ...
    sellValue(refundRate) { return Math.floor(this.totalInvested * refundRate); }
}
